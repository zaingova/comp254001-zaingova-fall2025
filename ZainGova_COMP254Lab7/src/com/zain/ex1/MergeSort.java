...
import com.zain.ex2.LinkedQueue;
import com.zain.ex2.Queue;
...
class MergeSort {
  ...
  public static <K> void mergeSortBottomUp(Queue<K> orig, Comparator<K> comp) {
      Queue<Queue<K>> qq = new LinkedQueue<>();

      while (!orig.isEmpty()) {
          Queue<K> single = new LinkedQueue<>();
          single.enqueue(orig.dequeue());
          qq.enqueue(single);
      }

      while (qq.size() > 1) {
          Queue<K> q1 = qq.dequeue();
          Queue<K> q2 = null;
          ...
          merge(q1, q2, merged, comp);

          qq.enqueue(merged);
      }

      if (!qq.isEmpty()) {
          Queue<K> sorted = qq.dequeue();
          while (!sorted.isEmpty()) {
              orig.enqueue(sorted.dequeue());
          }
      }
  }
