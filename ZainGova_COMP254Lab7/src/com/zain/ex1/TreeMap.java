...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  protected static class BalanceableBinaryTree<K,V>
    ...
    protected static class BSTNode<E> extends LinkedBinaryTree.Node<E> {
      int aux=0;
      BSTNode(E e, LinkedBinaryTree.Node<E> parent, LinkedBinaryTree.Node<E> leftChild, LinkedBinaryTree.Node<E> rightChild) {
    ...
    @Override
    protected LinkedBinaryTree.Node<Entry<K,V>> createNode(Entry<K,V> e, LinkedBinaryTree.Node<Entry<K,V>> parent,
                                                           LinkedBinaryTree.Node<Entry<K,V>> left, LinkedBinaryTree.Node<Entry<K,V>> right) {
      return new BSTNode<>(e, parent, left, right);
    }

    /** Relinks a parent node with its oriented child node. */
    private void relink(LinkedBinaryTree.Node<Entry<K,V>> parent, LinkedBinaryTree.Node<Entry<K,V>> child,
    ...
    public void rotate(Position<Entry<K,V>> p) {
      LinkedBinaryTree.Node<Entry<K,V>> x = validate(p);
      LinkedBinaryTree.Node<Entry<K,V>> y = x.getParent();        // we assume this exists
      LinkedBinaryTree.Node<Entry<K,V>> z = y.getParent();        // grandparent (possibly null)
      if (z == null) {
  ...
  private Position<Entry<K,V>> treeSearch(Position<Entry<K,V>> p, K key) {
    ...
    public Position<Entry<K,V>> treeSearchLoop(Position<Entry<K,V>> p, K key) {
